...
class bfoa_swarm2(intelligence.sw):
    """
...
#
    ...
    def __init__(self, n, function, lb, ub, dimension, iteration, Nre=16, Ned=4, Nc=2, Ns=12, C=0.1, Ped=0.25, Da=0.1, Wa=0.2, Hr=0.1, Wr=10, lamda=400):
        ...
        super(bfoa_swarm2, self).__init__()
        ...
        J_cc = np.array([0.0 for x in range(n)])
        J_ar = np.array([0.0 for x in range(n)])
        self._points(self.__agents)
        Pbest = self.__agents[J.argmin()]
        Gbest = Pbest
        ...
        for l in range(Ned):
            for k in range(Nre):
                ...
                J = np.array([function(x) for x in self.__agents])
                J_fit = np.array([J[x] for x in range(n)])  #[for p in J]
                print(J)
                print(J_cc)
                # self._points(self.__agents)
                Pbest = self.__agents[J.argmin()]
                ...
                for j in range(Nc):
                    ...
                    for i in range(n):
                        ...
                        #J_ar[i] = (np.exp( 1 + J_fit[i])-1)* J_cc[i]
                        J_ar[i] = np.exp(J_fit[i])* J_cc[i]
                        J[i] = (J_fit[i] + J_ar[i])

                        print(J_fit[i], J_cc[i], J_ar[i],
                              J[i], (J_fit[i] + J_ar[i]))
