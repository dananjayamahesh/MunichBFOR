...
def animation1D(agents, function, lb, ub, sr=False):
...
def animation(agents, function, lb, ub, dim=1, sr=False):
    ...
    if dim==1:
    	y = np.array([0.0 for j in agents[0]])
    else:
        y = np.array([j[1] for j in agents[0]])

    #y = np.array([j[1] for j in agents[0]])
    sc = plt.scatter(x, y, color='black', alpha=1, s=20)
...
def animation3D(agents, function, lb, ub, dim=1, sr=False):
...
def test_function_shape( function, lb, ub, dim=1):
...
#BACKUP
"""
def animation(agents, function, lb, ub,n=1, sr=False):
