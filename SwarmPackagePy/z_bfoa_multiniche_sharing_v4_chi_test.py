...
class z_bfoa_multiniche_sharing_v4_chi_test(intelligence.sw):
    """
...
#Chi square based assignment
    def points_to_clusters_assign(self):
        print('Point to Cluster Assignment')
        self.chi_capacity = [0 for i in range(self.trained_k+1)]
        for i in range(self.N):
            T = np.array(self.trained_peaks)
            T_diff = (T - self.__agents[i])
            T_diff_sq = T_diff**2
            T_sum = np.sum(T_diff_sq, axis=1)
            point_to_cent_mat = np.array(np.sqrt(T_sum))

            mini = point_to_cent_mat.argmin()
            minv = point_to_cent_mat[mini]

            #Big Decision to be made - How to allocate point to 
            threshold_dis = 1
            threshold_val = self.trained_peaks_fit[mini]*1.2 #not 80% percent, since this is min problem

            if(minv < threshold_val): #check 80% of the peak is achieved
                self.chi_capacity [mini]=  (self.chi_capacity [mini] +1)
            else:
                self.chi_capacity [self.trained_k] = (self.chi_capacity [self.trained_k] +1)
            #return [mini,minv]


    def __init__(self, n, function, lb, ub, dimension, iteration, Nre=16, Ned=4, Nc=2, Ns=12, C=0.1, Ped=0.25, Da=0.1, Wa=0.2, Hr=0.1, Wr=10, lamda=400, L=0.03, arga='none', argj='none', arged='false',sigma=1, d_min=1, d_max=3, clust_alpha=1, chi_test_enable = 'false', trained_centers=[], trained_centers_fit=[],kclus=0):
        """
        ...
        super(z_bfoa_multiniche_sharing_v4_chi_test, self).__init__()

        #Memory Usage
        self.cluster_centroids_mean = []
        self.cluster_centroids_var = []

        self.trained_peaks = trained_centers
        self.trained_peaks_fit = trained_centers_fit
        self.trained_k = 0

        if(chi_test_enable == 'true'):
            print('Chi Test Enabled')
            print(trained_centers)
            print(trained_centers_fit)
            kclus = len(trained_centers_fit)
            self.trained_k = kclus
            print('Number of Clusters :', kclus)

            fit_sum_tmp = sum([ (1/fit) for fit in trained_centers_fit])            

            for tmp in trained_centers_fit:
                self.cluster_centroids_mean += [(1/(tmp*fit_sum_tmp))*n]

            self.cluster_centroids_mean += [0.0]
            print('Cluster Means :', self.cluster_centroids_mean)

            for tmp_mean in self.cluster_centroids_mean:
                self.cluster_centroids_var += [tmp_mean*(1-(tmp_mean/n))]

            print('Cluster Variances :', self.cluster_centroids_var)
            self.cluster_centroids_var[kclus] = sum(self.cluster_centroids_var)
            print('Cluster Variances :', self.cluster_centroids_var)
        ...
        for l in range(Ned):
            ...
            for k in range(Nre):
                ...
                for j in range(Nc):
                    ...
                    self.points_to_clusters_assign()
                    print('Chi Sqaure', self.chi_capacity)                 
                    chi_val = np.sqrt(sum([(((self.chi_capacity[i] - self.cluster_centroids_mean[i])**2)/self.cluster_centroids_var[i]) for i in range(self.trained_k+1)]))
                    self.__CHIList.append(chi_val)
        ...
        """
    def _get_chilist(self):
        return self.__CHIList
    ...
    def _get_cluster_centroids_fit(self):
        return self.self.cluster_centroids_fit

    def _get_cluster_centroids_cap(self):
        return self.self.cluster_centroids_capacity  
