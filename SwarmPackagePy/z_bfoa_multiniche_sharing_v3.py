...
class z_bfoa_multiniche_sharing_v3(intelligence.sw):
    """
    VARIANT of V2 - J_SHared is a function of Jcc - Lets see
    ...
    def __init__(self, n, function, lb, ub, dimension, iteration, Nre=16, Ned=4, Nc=2, Ns=12, C=0.1, Ped=0.25, Da=0.1, Wa=0.2, Hr=0.1, Wr=10, lamda=400, L=0.03, arga='none', argj='none', arged='false',sigma=1):
        ...
        super(z_bfoa_multiniche_sharing_v3, self).__init__()
        ...
        for l in range(Ned):
            ...
            for k in range(Nre):
                ...
                #J_shared = [(J_fit[p]*self.niche_count[p]) for p in range(n)]
                J_shared = [(J_fit[p]*np.abs(J_cc[p])) for p in range(n)]
                """
                ...
                J_health_new = [(J_shared[p],p,J[p],J_cc[p],False) for p in range(n)]
                ...
                for p in range(n):
                    print(J_health_new[p],associated_niche[J_health_new[p][1]],associated_niche_peak[J_health_new[p][1]],niches[associated_niche[J_health_new[p][1]]][2])
