...
def animation_multipop(agents, function, lb, ub, n, seg, sr=False, ):
    ...
    colors = matplotlib.cm.rainbow(np.linspace(0, 1, seg))
    S = int(n/seg)
    #print('Start Animation',S)
    cs = [colors[(i//S)] for i in range(n)] 
    #print(cs)
    #sc = plt.scatter(x, y, color='black')
    sc = plt.scatter(x, y, color=cs)
...
def animation3D_multipop(agents, function, lb, ub, sr=False):
