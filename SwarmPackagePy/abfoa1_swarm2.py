...
class abfoa1_swarm2(intelligence.sw):
    """
...
#
    ...
    def __init__(self, n, function, lb, ub, dimension, iteration, Nre=16, Ned=4, Nc=2, Ns=12, C=0.1, Ped=0.25, Da=0.1, Wa=0.2, Hr=0.1, Wr=10, lamda=400):
        ...
        super(abfoa1_swarm2, self).__init__()
        ...
        J = np.array([function(x) for x in self.__agents])
        J_fit = np.array([J[x] for x in range(n)])
                    ...
                    for i in range(n):
                        ...
                        for m in range(Ns):
                            if(i == n / 2):
                                ...
                                print(C_a[i], J_fit[i], J_cc[i], J_ar[i],
                                      J[i], (J_fit[i] + J_cc[i]), J_best)
